...
const getMainPage = async (req, res) => {
  ...
  arrayOfMainPagePromises.push(
    Recipe.find({ category: "Breakfast" }, null, { limit })
      .sort({ name: 1 })
      .exec()
  );
  arrayOfMainPagePromises.push(
    Recipe.find({ category: "Miscellaneous" }, null, { limit })
      .sort({ name: 1 })
      .exec()
  );
  arrayOfMainPagePromises.push(
    Recipe.find({ category: "Chicken" }, null, { limit })
      .sort({ name: 1 })
      .exec()
  );
  arrayOfMainPagePromises.push(
    Recipe.find({ category: "Desserts" }, null, { limit })
      .sort({ name: 1 })
      .exec()
  );
